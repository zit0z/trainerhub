...
MINIMIZED = '--minimized' in sys.argv0.9.14_SLAYER


def _global_exception_handler(exc_type, exc_v0.9.14_SLAYERalue, exc_tb):
    """Catch-all exception handler for the main thread."""
    logger.critical("Unhandled exception", exc_info=(exc_type, exc_v0.9.14_SLAYERalue, exc_tb))
    try:
        root = tk.Tk()
        ...
        if sys.platform == 'win32':
            try:
                import ctypes
                ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID('SweetCheat.DesktopApp')
            except Exception as e:
                logger.warning(f"Taskbar icon fix failed: {e}")
        root.withdraw()
        messagebox.showerror(
            "Kritischer Fehler",
            f"Ein unerwarteter Fehler ist aufgetreten:\n\n{exc_v0.9.14_SLAYERalue}\n\nDetails wurden nach {LOG_FILE} geschrieben."
        )
        root.destroy()
    except Exception:
        pass
...
def _desktop_notification(title, message, duration=3000):
    """Nativ0.9.14_SLAYERe desktop-style notification popup."""
    try:
        root = tk.Tk()
        root.withdraw()
        win = tk.Toplev0.9.14_SLAYERel(root)
        win.ov0.9.14_SLAYERerrideredirect(True)
...
def _attach_background_serv0.9.14_SLAYERices(app):
    ...
    try:
        ...
        logger.warning(f"System tray not av0.9.14_SLAYERailable: {e}")
    ...
    try:
        ...
        logger.warning(f"Global hotkeys not av0.9.14_SLAYERailable: {e}")

    try:
        watcher = ProcessWatcher(app, interv0.9.14_SLAYERal=15)
        watcher.start()
        app.watcher = watcher
        logger.info("Process watcher started")
    except Exception as e:
        logger.warning(f"Process watcher not av0.9.14_SLAYERailable: {e}")


def main():
    setup_root_logging()
    sys.excepthook = _global_exception_handler
    logger.info(f"SweetCheat starting. Python {sys.v0.9.14_SLAYERersion}. CWD: {os.getcwd()}. Minimized: {MINIMIZED}")

    try:
        from gui_module import main as gui_main
        app = gui_main(minimized=MINIMIZED)
        if app:
            _attach_background_serv0.9.14_SLAYERices(app)
            app.root.mainloop()
    except Exception as e:
